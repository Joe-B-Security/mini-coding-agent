// rust_hook: Part 4 Rust-backed hook for mini-coding-agent.
//
// This crate exposes three Python-callable functions covering two
// benchmark workloads plus a dynamic-pattern API used by the scaling
// experiment:
//
...
#[pyfunction]
fn scan_regex(payload_json: &str) -> PyResult<String> {
...
#[pyfunction]
fn walk_ast(payload_json: &str) -> PyResult<String> {
    let parsed: serde_json::Value = match serde_json::from_str(payload_json) {
        Ok(v) => v,
        Err(_) => return Ok(String::from(r#"{"decision":"allow"}"#)),
    };
...
#[pyfunction]
fn set_patterns(patterns: Vec<String>) -> PyResult<()> {
...
#[pyfunction]
fn scan(haystack: &str) -> PyResult<usize> {
...
/// PyO3 module entry point. The function name here MUST match the
/// `[lib] name` in Cargo.toml and `tool.maturin.module-name` in
/// pyproject.toml, otherwise Python's import system can't find the
/// compiled extension.
#[pymodule]
fn rust_hook(_py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_function(wrap_pyfunction!(scan_regex, m)?)?;
    m.add_function(wrap_pyfunction!(walk_ast, m)?)?;
    m.add_function(wrap_pyfunction!(set_patterns, m)?)?;
